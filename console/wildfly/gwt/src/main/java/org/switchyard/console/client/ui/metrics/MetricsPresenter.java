...
public class MetricsPresenter extends Presenter<MetricsPresenter.MyView, MetricsPresenter.MyProxy> {
    ...
    @RuntimeExtension(name = NameTokens.RUNTIME_METRICS_TEXT, group = RuntimeGroup.METRICS, key = NameTokens.SUBSYSTEM)
    @RequiredResources(resources = {"/{selected.host}/{selected.server}/subsystem=switchyard" })
